...
class ECamera:
    ...
    def connect(self):
        ...
        try:
            ...
            self.setWindow()    # rewrite window settings
        except:
            self.alta_usb = None
            DEBUG(format_exc())
    ...
    def setBinning(self, binx, biny):
        ...
        if binx != self.binx or biny != self.biny:
            ...
            self.binx = binx
            self.biny = biny
            # set new window size based on new binning
            self.setWindow(sizex=sizex, sizey=sizey)

    def setWindow(self, x0=None, y0=None, sizex=None, sizey=None):
        '''
    ...
    def expose(self, exptime, filename=None):
        '''
...
filename=/export/images/ecam/UT120428/e0012.fits
        ...
        try:
            if not self.alta_usb:
                ...
                self.callback.exposureState('not available', 0.0, 0.0)
                return
            ...
            self.alta_usb.new_expose(self.callback.exposureState, self.integration, filename)
...
with Pyro4.core.Daemon(host="shack.apo.nmsu.edu", port=51000) as daemon:
    uri=daemon.register(ecamera,'ecamera')
    print 'server uri is ', uri
    print("Server ready.")
    daemon.requestLoop(loopCondition=lambda: loopCamera)
    print 'server quit'
