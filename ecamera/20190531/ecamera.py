...
#		self.image_directory = '/home/arc'
	...
	def _setup_image(self, line):
		...
		try:
			...
			DEBUG('_setup_image: ctrx %s, sizex %s' % (ctrx, sizex))
			...
			if self.y0 < 0:
				...
				self.y0 = 0
			DEBUG('_setup_image: ctrx %s, sizex/2 %s, x0 %s' % (ctrx, self.sizex/2, self.x0))
			...
			DEBUG('_setup_image: set binning to %s, %s\n' % (self.binx, self.biny), 0)
			if 'alta' == self.camera and self.alta_usb:
			...
			if 'alta' == self.camera and self.alta_usb:
				...
				# Image is rotated 270 degrees
				fli.setImageArea (self.fli_usb, (512 - (self.y0 + self.sizey * self.biny)), self.x0, self.sizey, self.sizex)
			else :
