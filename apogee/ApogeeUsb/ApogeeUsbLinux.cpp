...
#define APUSB_PID_FILTERWHEEL                   0x0100
...
struct usb_dev_handle	*g_hSysDriver[APN_USB_MAXCAMERAS];
struct usb_dev_handle	*g_hFilterDriver[APN_USB_MAXCAMERAS];
...
bool ApnUsbCreateRequest( unsigned short hDevice, unsigned char	Request,
							bool		InputRequest,
							unsigned short	Index,
							unsigned short	Value,
...
//	struct usb_dev_;
	...
	/* find ALTA device */
	for(bus = usb_busses; bus && !gotit; bus = bus->next) {
		for(dev = bus->devices; dev && !gotit; dev = dev->next) {
...
//	struct usb_dev_;
	...
	for(bus = usb_busses; bus && !gotit; bus = bus->next) {
		for(dev = bus->devices; dev && !gotit; dev = dev->next) {
			if (dev->descriptor.idVendor == USB_ALTA_VENDOR_ID && 
			     (dev->descriptor.idProduct == USB_FILTERWHEEL_PRODUCT_ID) ) {
				found = found+1;
				if (found == DevNumber) {
					hDevice = usb_open(dev);
					firmwareRevision = dev->descriptor.bcdDevice;
                                        idProduct = dev->descriptor.idProduct;
//					cerr << "Found Apogee FilterWHeel. Attempting to open... ";
					if (hDevice) {
                                            gotit = 1;
//					if (!usb_get_string_simple(hDevice, 
//						dev->descriptor.iSerialNumber, 
...
APN_USB_TYPE ApnUsbClose( unsigned short hDevice )
{
	...
	if ( g_hSysDriver[hDevice] != 0 )
	{
		usb_release_interface(g_hSysDriver[hDevice], 0x0);
		usb_close(g_hSysDriver[hDevice]);
		g_hSysDriver[hDevice] = 0;
	}
...
APN_USB_TYPE ApnUsbReadReg( unsigned short hDevice,  unsigned short FpgaReg, unsigned short *FpgaData )
{
    int Success;
...
APN_USB_TYPE ApnUsbWriteReg( unsigned short hDevice,  unsigned short FpgaReg, unsigned short FpgaData )
{
    char *cbuf;
    int Success;
...
APN_USB_TYPE ApnUsbWriteRegMulti( unsigned short hDevice,   unsigned short FpgaReg, unsigned short FpgaData[], unsigned short RegCount )
{
...
APN_USB_TYPE ApnUsbWriteRegMultiMRMD( unsigned short hDevice,   unsigned short FpgaReg[], 
				      ...
				      unsigned short RegCount )
{
	...
	for ( Counter=0; Counter<RegCount; Counter++ )
	{
		if ( ApnUsbWriteReg(hDevice, FpgaReg[Counter], 
...
APN_USB_TYPE ApnUsbReadStatusRegs( unsigned short hDevice, bool UseAdvStatus,
				   bool *DoneFlag,
				   ...
				   unsigned short *CurrentFrame)
{
	...
	if (UseAdvStatus) {
		...
		Success = usb_control_msg(g_hSysDriver[hDevice],
...
APN_USB_TYPE ApnUsbStartExp( unsigned short hDevice, unsigned short ImageCount , unsigned short ImageWidth,
...
APN_USB_TYPE ApnUsbStopExp(  unsigned short hDevice,  bool DigitizeData )
{
	BOOLEAN Success;
	unsigned short	BytesReceived;
...
APN_USB_TYPE ApnUsbStopCI( unsigned short hDevice, unsigned short PostStopCount ) 
{ 
	BOOLEAN Success; 
	ULONG	BytesReceived; 
...
//	}
	...
	for ( Counter=0; Counter<LoopCount; Counter++ )
	{
		...
		ReceivedSize = usb_bulk_read(g_hSysDriver[hDevice], 0x86, 
				 (char *)pMem, IMAGE_BUFFER_SIZE, 2000);
...
//        printf("DRIVER: bulkread size=%x\n",ReceivedSize);
	...
	{
		ReceivedSize = usb_bulk_read(g_hSysDriver[hDevice], 0x86, 
				(char *)pMem, Remainder, 2000);
        //printf("DRIVER: bulkread2 size=%x,%x\n",Remainder,ReceivedSize);
...
APN_USB_TYPE ApnUsbSysDriverVersion( unsigned short hDevice, double *VersionNumber )
{
...
/*
   		Success = usb_control_msg((struct usb_dev_handle *)g_hSysDriver[hDevice], 
                                USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, VND_APOGEE_EEPROM,
				(unsigned short)( (APN_USB_SN_EEPROM_BANK<<8) | APN_USB_SN_EEPROM_CHIP ),
                                APN_USB_SN_EEPROM_ADDR, 
				(char *)&pBuffer,APUSB_CUSTOM_SN_BYTE_COUNT, 10000);
 */
	        Success = usb_control_msg((struct usb_dev_handle *)g_hSysDriver[hDevice], 
                                USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, VND_APOGEE_CAMCON_REG,
...
APN_USB_TYPE ApnUsbRead8051FirmwareRevision(unsigned short hDevice, 
											 char		*Revision ) 
{ 
	BOOLEAN	Success; 
...
APN_USB_TYPE ApnUsbRead8051FilterFirmwareRevision(unsigned short hDevice, 
											 char		*Revision ) 
{ 
	BOOLEAN	Success; 
	char	pBuffer[APN_USB_8051_REV_BYTE_COUNT+1]; 
	...
	if ( (idProduct == APUSB_PID_ALTA) && (firmwareRevision >=  APUSB_8051_REV_DID_SUPPORT)) {
		...
		Length = APN_USB_8051_REV_BYTE_COUNT+1; 
   	        Success = usb_control_msg((struct usb_dev_handle *)g_hFilterDriver, 
                                USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, VND_APOGEE_CAMCON_REG,
                                            2, 0,(char *)&pBuffer, APUSB_8051_REV_BYTE_COUNT, 10000);
...
*/
   	Success = usb_control_msg((struct usb_dev_handle *)g_hFilterDriver, 
                                ...
                                TempValue, 0,(char *)&TempValue, 0, 10000);
