									   ...
									   unsigned long	Option )
{
	...
	ApnUsbReadVendorInfo( m_cameraNumber, &m_pvtVendorId, &m_pvtProductId, &m_pvtDeviceId );
								 ...
								 unsigned long	Option )
{
	...
	ApnUsbSysDriverVersion( m_cameraNumber,  &m_SysDriverVersion );
	...
	if ( InitDefaults() != 0 )
    {
        AltaDebugOutputString("usb: init defaults failed\n");
		return false;
    }

	// Done
	AltaDebugOutputString( "APOGEE.DLL - CApnCamera::InitDriver() -> END" );
    AltaDebugOutputString("usb: init defaults okay\n");
...
long CApnCamera::GetCameraSerialNumber( char *CameraSerialNumber, long *BufferLength )
{
	...
	{
		if ( ApnUsbReadCustomSerialNumber( m_cameraNumber,  pBuffer, &Length ) != APN_USB_SUCCESS )
		{
			strcpy( CameraSerialNumber, "Unknown" );
			*BufferLength = strlen( CameraSerialNumber );
		}
...
long CApnCamera::GetUsb8051FirmwareRev( char *FirmwareRev, long *BufferLength )
{
	...
	ApnUsbRead8051FirmwareRevision( m_cameraNumber,  FirmwareRev );
								   ...
								   unsigned long  &Count )
{
	...
	unsigned short	PadPixels;
	unsigned long	DownloadHeight;
	unsigned long	DownloadSizeInBytes;
	unsigned long	*pImageTransferBytes;
        unsigned long   tbytes;
	unsigned long	i, j;
	char			szOutputText[128];


    AltaDebugOutputString ("GetImageData\n");
	if ( m_pvtConnectionOpen == false )
		return CAPNCAMERA_ERR_CONNECT;

	if ( !ImageInProgress() )
    {
        AltaDebugOutputString ("GetImageData: not image in progress, return\n");
	...
	if ( (m_pvtNumImages == 1) || (m_pvtExposeSequenceBulkDownload) )
	{
		...
		{
			Sleep( 50 );
			AltaDebugPrint(szOutputText, "GetImageData: waiting for image ready %d\n", read_ImagingStatus());
            AltaDebugOutputString(szOutputText);
		}
	}

	pImageTransferBytes = &tbytes;
    ...
    AltaDebugOutputString("GetImageData: here\n");
	...
	if ( read_PlatformType() == Apn_Platform_Alta )
	{
		...
		DownloadSizeInBytes = m_SysImgSizeBytes;

        AltaDebugPrint(szOutputText, "GetImageData: w %d\n", Width);
        AltaDebugOutputString(szOutputText);
        AltaDebugPrint(szOutputText, "              h %d\n", Height);
        AltaDebugOutputString(szOutputText);
        AltaDebugPrint(szOutputText, "         offset %d\n", Offset);
        AltaDebugOutputString(szOutputText);
	}
	else
	{
		PadPixels = ((Width+Offset) * DownloadHeight) % 8;

		pTempBuffer = new unsigned short[((Width+Offset) * DownloadHeight) + PadPixels];

		DownloadSizeInBytes = m_SysImgSizeBytes + (PadPixels * 2);
	}
	
	if ( ApnUsbGetImage( m_cameraNumber, DownloadSizeInBytes, pTempBuffer, pImageTransferBytes ) != APN_USB_SUCCESS )
	{
		...
		m_pvtConnectionOpen = false;
        AltaDebugOutputString("GetImageData Failed!!\n");
    ...
    AltaDebugOutputString("GetImageData move data\n");
    ...
    AltaDebugPrint(szOutputText, "GetImageData: readout count %d\n", Count);
    AltaDebugOutputString(szOutputText);
	...
	else
	{
		if ( (m_pvtNumImages == 1) || (m_pvtExposeSequenceBulkDownload) )
		{
			...
			SignalImagingDone();
			AltaDebugOutputString( "Single Image  SignalImagingDone is done" );
		}
    ...
    AltaDebugOutputString("GetImageData is successful, bye\n");
								  ...
								  unsigned short &Size )
{
	...
	if ( ApnUsbGetImage( m_cameraNumber,  BytesPerLine, pTempBuffer, pImageTransferBytes ) != APN_USB_SUCCESS )
	{
		ApnUsbClose(m_cameraNumber );
...
// Adjust for binning
//		if ( m_pvtExposeBitsPerPixel == 16 &&  (( m_pvtExposeHBinning > 1 ) || ( vbin > 1)) )
//			m_pvtExposeWidth -= 1;
	...
	{
		if ( ApnUsbStartCI( m_cameraNumber,  m_pvtExposeWidth, m_pvtExposeHeight ) != APN_USB_SUCCESS )
		{
			ApnUsbClose( m_cameraNumber );
	...
	else
	{
		if ( m_pvtExposeSequenceBulkDownload == true )
		{
			if ( ApnUsbStartExp( m_cameraNumber,  1, m_pvtExposeWidth, (m_pvtExposeHeight*m_pvtNumImages) ) != APN_USB_SUCCESS )
			{
				ApnUsbClose( m_cameraNumber );
		...
		else
		{
			...
			if ( ApnUsbStartExp( m_cameraNumber,  m_pvtNumImages, m_pvtExposeWidth, m_pvtExposeHeight ) != APN_USB_SUCCESS )
			{
				ApnUsbClose(m_cameraNumber );
...
long CApnCamera::PostStopExposure( bool DigitizeData )
{
	...
	switch( m_pvtExposeCameraMode )
	{
		case Apn_CameraMode_Normal:
			...
			if ( !read_ExposureTriggerGroup() && !read_ExposureTriggerEach() && !read_ExposureExternalShutter() )
			{
				...
				if ( !DigitizeData )
				{
					...
					if ( ApnUsbGetImage( m_cameraNumber,  m_SysImgSizeBytes, pRequestData , &transferbytes) != APN_USB_SUCCESS )
					{
						ApnUsbClose(m_cameraNumber );
			...
			else
			{
				...
				if ( read_ImagingStatus() == Apn_Status_WaitingOnTrigger )
				{
					...
					ApnUsbStopExp( m_cameraNumber,  false );
					SignalImagingDone();
					ResetSystem();
				}
				else
		...
		case Apn_CameraMode_TDI:
			...
			ApnUsbStopExp( m_cameraNumber,  DigitizeData );
...
long CApnCamera::Read( unsigned short reg, unsigned short& val )
{
	...
	if ( ApnUsbReadReg( m_cameraNumber,  reg, &val ) != APN_USB_SUCCESS )
	{
		ApnUsbClose(m_cameraNumber  );
...
long CApnCamera::Write( unsigned short reg, unsigned short val )
{
	...
	if ( ApnUsbWriteReg( m_cameraNumber,  reg, val ) != APN_USB_SUCCESS )
	{
		ApnUsbClose(m_cameraNumber );
...
long CApnCamera::WriteMultiSRMD( unsigned short reg, unsigned short val[], unsigned short count )
{
	...
	if ( ApnUsbWriteRegMulti( m_cameraNumber,  reg, val, count ) != APN_USB_SUCCESS )
	{
		ApnUsbClose(m_cameraNumber );
...
long CApnCamera::WriteMultiMRMD( unsigned short reg[], unsigned short val[], unsigned short count )
{
	...
	if ( ApnUsbWriteRegMultiMRMD( m_cameraNumber,  reg, val, count ) != APN_USB_SUCCESS )
	{
		ApnUsbClose(m_cameraNumber );
...
//	AltaDebugOutputString( "APOGEE.DLL - CApnCamera::QueryStatusRegs()" );
	...
	if ( ApnUsbReadStatusRegs( m_cameraNumber, 
...
long CApnCamera::SetCameraSerialNumber( char *CameraSerialNumber, long *BufferLength )
{
	AltaDebugOutputString( "APOGEE.DLL - CApnCamera::SetCameraSerialNumber()" );
	...
	if ( m_pvtDeviceId < 0x0011 )
	{
		return CAPNCAMERA_ERR_SN;
                ...
                Length = (unsigned short)*BufferLength;
		if ( ApnUsbWriteCustomSerialNumber( m_cameraNumber, CameraSerialNumber , &Length ) != APN_USB_SUCCESS )
		{
			return CAPNCAMERA_ERR_SN;
		}
	}
...
bool CApnCamera::DownloadFirmware( PINTEL_HEX_RECORD Firmware )
{
	AltaDebugOutputString( "APOGEE.DLL - CApnCamera::DownloadFirmware()" );

	if ( ApnUsb8051Reset( m_cameraNumber, 1 ) ) 
	{
		if ( ApnUsbDownloadIntelHex( m_cameraNumber, Firmware ) )
			ApnUsb8051Reset(m_cameraNumber,  0 );
		else
			return false;
	} else {
		return false;
