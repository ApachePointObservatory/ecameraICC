...
bool CApnCamera::Expose( double Duration, bool Light )
{
	...
	if ( m_pvtCameraPoweredDown )
	{
		AltaDebugOutputString( "APOGEE.DLL - CApnCamera::Expose() -> ERROR: System Powered Down!!" );
		return false;
	}

	if ( GetCameraInterface() != Apn_Interface_TEST )
	{
		WaitCounter = 0;
	...
	m_pvtExposureImageCount		= read_ImageCount();
	m_pvtExposureBulkDownload	= read_SequenceBulkDownload();
...
bool CApnCamera::TakeNoDataDarkExposure( double Duration )
{
	unsigned short	RegVal;
	unsigned long	DarkTime;
	unsigned long	WaitCounter;
	bool			tdiGroupTrigger;
	bool			tdiIndTrigger;
	bool			normalGroupTrigger;
	bool			normalIndTrigger;


	// trigger var inits
	tdiGroupTrigger		= false;
	tdiIndTrigger		= false;
	normalGroupTrigger	= false;
	normalIndTrigger	= false;

	// see if any triggers are enabled
	if ( m_pvtTriggerNormalGroup )
	{
		normalGroupTrigger = true;
		write_TriggerNormalGroup( false );
	}

	if ( m_pvtTriggerNormalEach )
	{
		normalIndTrigger = true;
		write_TriggerNormalEach( false );
	}

	if ( m_pvtTriggerTdiKineticsGroup )
	{
		tdiGroupTrigger = true;
		write_TriggerTdiKineticsGroup( false );
	}

	if ( m_pvtTriggerTdiKineticsEach )
	{
		tdiIndTrigger = true;
		write_TriggerTdiKineticsEach( false );
	}

	// disable fpga fifo writes
	Read( FPGA_REG_OP_B, RegVal );
	RegVal |= FPGA_BIT_FIFO_WRITE_BLOCK;
	Write( FPGA_REG_OP_B, RegVal );

	// Calculate the exposure time to program to the camera
	DarkTime = ((unsigned long)(Duration / m_PlatformTimerResolution)) + m_PlatformTimerOffsetCount;

	Write( FPGA_REG_TIMER_LOWER, (unsigned short)(DarkTime & 0xFFFF));
	DarkTime = DarkTime >> 16;
	Write( FPGA_REG_TIMER_UPPER, (unsigned short)(DarkTime & 0xFFFF));

	// start the dark exposure 
	Write( FPGA_REG_COMMAND_A, FPGA_BIT_CMD_DARK );

	m_pvtImageInProgress = true;
	m_pvtImageReady		 = false;

	WaitCounter = 0;
	while ( read_ImagingStatus() != Apn_Status_ImageReady )
	{
		Sleep( 20 );
	...
	if ( normalGroupTrigger )
		write_TriggerNormalGroup( true );

	if ( normalIndTrigger )
		write_TriggerNormalEach( true );

	if ( tdiGroupTrigger )
		write_TriggerTdiKineticsGroup( true );

	if ( tdiIndTrigger )
		write_TriggerTdiKineticsEach( true );

	return true;
}

void CApnCamera::IssueExposeCommand( bool Light )
{
	unsigned short	RegStartCmd;
	char			szOutputText[128];


	sprintf( szOutputText, "APOGEE.DLL - CApnCamera::IssueExposeCommand( Light = %d ) -> BEGIN", Light );
	AltaDebugOutputString( szOutputText );
	...
	case Apn_CameraMode_TDI:
		...
		if ( !Light && !read_DisableShutter() )
		{
			write_DisableShutter( true );

			m_pvtRestoreShutterControl = true;
		}
		
		m_pvtExposureTDIRows = m_pvtTDIRows;

		if ( m_pvtTriggerTdiKineticsEach )
		{
			Write( FPGA_REG_TDI_ROWS, 0x0 );

			m_pvtRestoreTdiRowCount = true;
		}
	...
	// Send the instruction sequence to the camera
	AltaDebugOutputString( "APOGEE.DLL - CApnCamera::IssueExposeCommand() -> Issue start command to FPGA_REG_COMMAND_A" );
	Write( FPGA_REG_COMMAND_A, RegStartCmd );
}

void CApnCamera::PostExposureRestoreParams()
{
	AltaDebugOutputString( "APOGEE.DLL - CApnCamera::PostExposureRestoreParams()" );

	if ( m_pvtRestoreShutterControl )
	{
		write_DisableShutter( false );

		m_pvtRestoreShutterControl = false;
	}

	if ( m_pvtRestoreTdiRowCount )
	{
		Write( FPGA_REG_TDI_ROWS, m_pvtExposureTDIRows );

		m_pvtRestoreTdiRowCount = false;
	}
}
...
bool CApnCamera::PowerDown()
{
	// if we are currently imaging, don't power down
	if ( m_pvtImageInProgress )
		return false;

	// steps for a system power down/sleep mode
	// 1)  turn off cooler, if it is on
	// 2)  close shutter, if it is open
	// 3)  turn fans off
	// 4)  reset controller without flushing

	m_pvtPowerDownCoolerEnable	= read_CoolerEnable();
	m_pvtPowerDownFanMode		= read_FanMode();

	if ( read_CoolerEnable() )
		write_CoolerEnable( false );

	if ( read_ForceShutterOpen() )
		write_ForceShutterOpen( false );

	write_FanMode( Apn_FanMode_Off );

	ResetSystemNoFlush();

	m_pvtCameraPoweredDown = true;

	return true;
}

bool CApnCamera::Resume()
{
	if ( m_pvtCameraPoweredDown )
	{
		ResetSystem();

		write_FanMode( m_pvtPowerDownFanMode );

		write_CoolerEnable( m_pvtPowerDownCoolerEnable );
	}
...
void CApnCamera::read_CameraModel( char *CameraModel, long *BufferLength )
{
	char szModel[256];
...
void CApnCamera::write_CoolerEnable( bool CoolerEnable )
{
	...
	if ( m_pvtCameraPoweredDown )
	{
		AltaDebugOutputString( "APOGEE.DLL - CApnCamera::write_CoolerEnable() -> System Powered Down!" );
		return;
	}
...
double CApnCamera::read_CoolerSetPoint()
{
    ...
    //used y=mx+b to figure out the input register value, where y is the register value
    //now using known y to solve for x, see write_CoolerSetPoint for more details.
	TempVal = ( (RegVal - m_PlatformTempSetpointIntercept) / m_PlatformTempSetpointSlope ) 
        * m_PlatformTempDegreesPerBit;

	return TempVal;
}

void CApnCamera::write_CoolerSetPoint( const double SetPoint )
{
	char szOutputText[128];
	...
	double TempVal = SetPoint;
	
	if ( SetPoint < m_PlatformTempSetpointMin )
    {
        //ticket #18
        //min temp value is -60.0
        //anything greater than that results in a negative number 
        //when plugged into
        //(TempVal / m_PlatformTempDegreesPerBit) + m_PlatformTempSetpointZeroPoint
        //the cowboy cast then causes weird values to be entered into the 
        //desired temp register
		TempVal = m_PlatformTempSetpointMin;
    }

	if ( SetPoint >  m_PlatformTempSetpointMax)
    {
    ...
    double x = TempVal / m_PlatformTempDegreesPerBit;

	unsigned short RegVal = (unsigned short)( (m_PlatformTempSetpointSlope*x)  + m_PlatformTempSetpointIntercept );
...
void CApnCamera::write_CoolerBackoffPoint( const double BackoffPoint )
{
	char szOutputText[128];
	...
	if ( BackoffPoint < m_PlatformTempBackoffMin )
    {
		TempVal = m_PlatformTempBackoffMin;
    }

	if ( BackoffPoint > m_PlatformTempBackoffMax )
    {
		TempVal = m_PlatformTempBackoffMax;
    }
...
void CApnCamera::write_FanMode( Apn_FanMode FanMode )
{
	...
	if ( m_pvtCameraPoweredDown )
	{
		AltaDebugOutputString( "APOGEE.DLL - CApnCamera::write_FanMode() -> System Powered Down!!" );
		return;
	}
...
{
	return( Apn_Resolution_SixteenBit == m_pvtDataBits ? 
        m_ApnSensorInfo->m_RoiPatternSixteen.BinningLimit :
        m_ApnSensorInfo->m_RoiPatternTwelve.BinningLimit ); 
}
...
unsigned short CApnCamera::read_ExposureImageCount()
{
	return m_pvtExposureImageCount;
}

bool CApnCamera::read_ExposureBulkDownload()
{
	return m_pvtExposureBulkDownload;
}
...
void CApnCamera::write_TDIRows( unsigned short TdiRows )
{
	...
	Write( FPGA_REG_TDI_ROWS, TdiRows );
...
bool CApnCamera::read_TriggerNormalEach()
{
    ...
    return ( (RegVal & FPGA_BIT_IMAGE_TRIGGER_EACH) ? true : false );	
}
...
bool CApnCamera::read_TriggerNormalGroup()
{
	...
	unsigned short RegVal;
	Read( FPGA_REG_OP_C, RegVal );
	return ( (RegVal & FPGA_BIT_IMAGE_TRIGGER_GROUP) ? true : false );	
}
...
bool CApnCamera::read_TriggerTdiKineticsEach()
{
	...
	unsigned short RegVal;
	Read( FPGA_REG_OP_C, RegVal );
	return ( (RegVal & FPGA_BIT_TDI_TRIGGER_EACH) ? true : false );	
}
...
void CApnCamera::write_TriggerTdiKineticsEach( bool TriggerTdiKineticsEach )
{
	...
	// if we're trying to enable individual TDI/kinetics triggers, disable group triggers
	if ( TriggerTdiKineticsEach && m_pvtTriggerTdiKineticsGroup )
		write_TriggerTdiKineticsGroup( false );
...
bool CApnCamera::read_TriggerTdiKineticsGroup()
{
	...
	unsigned short RegVal;
	Read( FPGA_REG_OP_C, RegVal );
	return ( (RegVal & FPGA_BIT_TDI_TRIGGER_GROUP) ? true : false );
}
...
void CApnCamera::write_TriggerTdiKineticsGroup( bool TriggerTdiKineticsGroup )
{
	...
	// if we're trying to enable group TDI/kinetics triggers, disable individual triggers
	if ( TriggerTdiKineticsGroup && m_pvtTriggerTdiKineticsEach )
		write_TriggerTdiKineticsEach( false );
...
bool CApnCamera::read_PreFlashEnable()
{
	return m_pvtPreFlashEnable;
}

void CApnCamera::write_PreFlashEnable( bool PreFlashEnable )
{
	m_pvtPreFlashEnable = PreFlashEnable;
}
...
unsigned short CApnCamera::read_IoPortBlankingBits()
{
	return m_pvtIoPortBlankingBits;
}

void CApnCamera::write_IoPortBlankingBits( unsigned short IoPortBlankingBits )
{
	if ( m_pvtPlatformType == Apn_Platform_Alta )
	{
		IoPortBlankingBits &= FPGA_MASK_IO_PORT_BLANKING_BITS;
		Write( FPGA_REG_IO_PORT_BLANKING_BITS, IoPortBlankingBits );
		m_pvtIoPortBlankingBits = IoPortBlankingBits;
	}
}
...
long CApnCamera::LookupAltaCameraId( unsigned short CameraId )
{
	...
	case APN_ALTA_CCD7700D9_CAM_ID:
		m_ApnSensorInfo = new CApnCamData_CCD7700D9;
		break;
	default:
		return 1;
		break;
...
//        printf("DefaultOffsetTwelveBit = %u\n",m_ApnSensorInfo->m_DefaultOffsetTwelveBit);
	...
	// If we are a USB2 camera, set all the A/D processor(s)
	if ( GetCameraInterface() == Apn_Interface_USB )
	{
		if ( m_ApnSensorInfo->m_PrimaryADType == ApnAdType_Ascent_Sixteen )
		{
			// left side
			Read( FPGA_REG_OP_D, RegVal );
			RegVal &= ~FPGA_BIT_AD_LOAD_SELECT_0;
			Write( FPGA_REG_OP_D, RegVal );
			...
			// FPGA_BIT_AD_LOAD_SELECT_0 bit back to zero
			Read( FPGA_REG_OP_D, RegVal );
			RegVal &= ~FPGA_BIT_AD_LOAD_SELECT_0;
			Write( FPGA_REG_OP_D, RegVal );
	...
	// Set the IR PreFlash vars
	write_PreFlashEnable( false );
	...
	// On write_FanMode return, our state will be Apn_FanMode_Low
	...
	// Set our initial power down state settings to something sensible
	m_pvtPowerDownCoolerEnable	= read_CoolerEnable();
	m_pvtPowerDownFanMode		= read_FanMode();

	// Init all of our other vars
	m_pvtCameraPoweredDown		= false;
	...
	// Final inits for variables set during the Expose() method.  Init them to something
	// safe, even though they shouldn't be accessed until the Expose() is issued
	m_pvtExposurePixelsV		= m_pvtRoiPixelsV;
	m_pvtExposurePixelsH		= m_pvtRoiPixelsH;
...
void CApnCamera::SetPlatformConstants()
{
	if ( read_PlatformType() == Apn_Platform_Alta )
	{
		...
		m_PlatformVBinningMax				= APN_VBINNING_MAX_ALTA;
        ...
        m_PlatformTempSetpointSlope         = APN_TEMP_SETPOINT_SLOPE_ALTA;
        m_PlatformTempSetpointIntercept     = APN_TEMP_SETPOINT_INTERCEPT_ALTA;


        m_PlatformTempBackoffMin            = APN_TEMP_BACKOFF_MIN_ALTA;
        m_PlatformTempBackoffMax            = APN_TEMP_BACKOFF_MAX_ALTA;
		...
		m_PlatformPreflashDuration			= APN_PREFLASH_DURATION_ALTA;
	}
	else if ( read_PlatformType() == Apn_Platform_Ascent )
	{
		m_PlatformHBinningMax				= APN_HBINNING_MAX_ASCENT;
		m_PlatformVBinningMax				= APN_VBINNING_MAX_ASCENT;
        ...
        m_PlatformTempSetpointSlope         = APN_TEMP_SETPOINT_SLOPE_ASCENT;
        m_PlatformTempSetpointIntercept     = APN_TEMP_SETPOINT_INTERCEPT_ASCENT;


        m_PlatformTempBackoffMin            = APN_TEMP_BACKOFF_MIN_ASCENT;
        m_PlatformTempBackoffMax            = APN_TEMP_BACKOFF_MAX_ASCENT;
		...
		m_PlatformPreflashDuration			= APN_PREFLASH_DURATION_ASCENT;
	}
}
