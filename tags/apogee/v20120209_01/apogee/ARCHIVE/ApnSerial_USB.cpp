							   ...
							   unsigned short SerialId )
{
        ...
        if ( (ApnUsbSysDriverVersion( m_CamIdA, &VersionNum) != APN_USB_SUCCESS) ||
                 (ApnUsbReadVendorInfo(m_CamIdA,  &VendorId, &ProductId, &DeviceId) != APN_USB_SUCCESS) )
        {
                ClosePort();
                return false;
        }
...
bool CApnSerial::GetBaudRate(unsigned long *BaudRate )
{
        ...
        if ( ApnUsbSerialReadBaudRate(m_CamIdA,  m_SerialId, BaudRate)
!= APN_USB_SUCCESS )
        {
                *BaudRate = 0;
...
bool CApnSerial::SetBaudRate(unsigned long BaudRate )
{
        ...
        if ( ApnUsbSerialWriteBaudRate( m_CamIdA,  m_SerialId, BaudRate ) != APN_USB_SUCCESS )
        {
                return false;
        }
...
bool CApnSerial::GetFlowControl( Apn_SerialFlowControl *FlowControl )
{
        ...
        if ( ApnUsbSerialReadFlowControl( m_CamIdA, m_SerialId, &EnableFlowControl) != APN_USB_SUCCESS )
        {
...
bool CApnSerial::SetFlowControl( Apn_SerialFlowControl FlowControl )
{
        ...
        if ( ApnUsbSerialWriteFlowControl( m_CamIdA, m_SerialId, EnableFlowControl) != APN_USB_SUCCESS )
        {
                return false;
        }
...
bool CApnSerial::GetParity( Apn_SerialParity *Parity )
{
        ...
        if ( ApnUsbSerialReadParity(m_CamIdA,  m_SerialId, &ParityRead) != APN_USB_SUCCESS )
        {
...
bool CApnSerial::SetParity( Apn_SerialParity Parity )
{
        ...
        if ( ApnUsbSerialWriteParity( m_CamIdA, m_SerialId, ParityWrite) != APN_USB_SUCCESS )
        {
                return false;
        }
...
bool CApnSerial::Read(int dummy,char	*ReadBuffer, unsigned short *ReadCount )
{
        ...
        if ( ApnUsbSerialRead( m_CamIdA, m_SerialId, ReadBuffer, ReadCount) != APN_USB_SUCCESS )
        {
                *ReadCount      = 0;
...
bool CApnSerial::Write(int dummy, char *WriteBuffer, unsigned short WriteCount )
{
        ...
        if ( ApnUsbSerialWrite(m_CamIdA,  m_SerialId, WriteBuffer, WriteCount) != APN_USB_SUCCESS )
        {
                return false;
        }
